...
class WeiboSpiderSpider(scrapy.Spider):
    ...
    url = "http://weibo.cn/"
    start_urls = ['1735261917']  # 爬取入口微博ID
    ...
    def account_parse(self, response):
        ...
        counts = sel.xpath(
            "//div[@class='u']/div[@class='tip2']").extract_first()
        ...
        if int(item['tweet_stats']) < 4500 and int(item['following_stats']) > 1000 and int(item['follower_stats']) < 500:
            raise scrapy.exceptions.CloseSpider("僵尸粉")
    ...
    def profile_parse(self, response):
        ...
        sel = scrapy.Selector(response)
        info = sel.xpath(
            "//div[@class='tip']/following-sibling::div[@class='c']").extract_first()
        item["profile_pic"] = sel.xpath(
            "//div[@class='c']/img/@src").extract_first()
        item["nick_name"] = re.findall(u'昵称:(.*?)<br>', info)[0]
        item["sex"] = re.findall(
            u'性别:(.*?)<br>', info) and re.findall(u'性别:(.*?)<br>', info)[0] or ''
        item["location"] = re.findall(
            u'地区:(.*?)<br>', info) and re.findall(u'地区:(.*?)<br>', info)[0] or ''
        item["birthday"] = re.findall(
            u'生日:(.*?)<br>', info) and re.findall(u'生日:(.*?)<br>', info)[0] or ''
        item["bio"] = re.findall(
            u'简介:(.*?)<br>', info) and re.findall(u'简介:(.*?)<br>', info)[0] or ''
        yield item
